...
%	\red{NDG: here starts the more precise stuff. note: the notion of emotion attribution and tie ins to ToE should have already appeared. need to be more precise about what you mean by causally-relevant.}
	...
	Second, why might observers factor near misses into their affective cognition even for random outcomes, over which agents clearly have no control?  One possibility is that even in such situations, observers might irrationally believe that agent exerts some control over their outcomes.  Such ``illusions of control" are in fact common. For instance, people overestimate the amount of control they have over random events such as having more confidence in winning when they chose a lottery ticket number than when they were assigned one \cite{Langer1975, Taylor1988}. To the extent that observers indeed ascribe illusory control to agents, observers should incorporate near misses into their emotion attributions even in cases that are demonstrably random.  

	
	In Experiment 2, we explored this possibility by manipulating a game with random outcomes to produce two situations which we believed would be produce illusions of control in two different ways. In Version A of the game, observers saw an agent guess the \textit{location} of a single ``target" card with a known number (``Guess where the 10 card is") amidst a large array of cards. In Version B, agents had to guess the \textit{number} behind a target card (``Guess what number is behind this card"). We crossed this manipulation with whether agents' choices were spatially and / or numerically close to the actual target. In all cases, observers believed that the location (in Version A) or number (in Version B) of the target card was chosen at random, and thus agents had no \textit{actual control} over their chances of winning. As discussed above, people choosing the number on their lottery tickets feel more confident in winning, presumably because their choice of number gives them an illusion of control over an otherwise random outcome. Similarly, we reasoned that, in Version A when agents chose the locations of their cards, observers might nonetheless attribute illusory control over the cards' location to the agents. Hence, observers would be more likely to judge that an agent whose choice was spatially close to the target card would feel worse than another agent whose card was spatially distant.  By contrast, in Version B when agents guessed numbers, observers might imbue that agent with illusory control over the numbers behind the cards. If this is the case, observers should attribute more negative affect to an agent when he is numerically close, versus far, from the correct answer. The results of Experiment 2 support this hypothesis of illusory control impacting near-miss judgments.
...
%\red{JZ: How about generating a 2x2 table that explains, for each situation, what the agent controlled, and how close they were on the controlled, and uncontrolled dimension?}
	...
	In the Choose-Number (\textit{Number}) condition, participants were presented with a game with slightly different rules. There were the same 20 cards, and a target card (circled in purple), all face-down. Characters had to guess the number behind the target card. After picking a number, the card with the picked number was revealed. The characters were the same as in the \textit{Position} condition, i.e., on ``Close Distance vs. Close Number" trials, Scott picked the number 19 (\textbf{Close distance}) and Frank picked 11 (\textbf{Close Number}). After the character made their guesses, the winning number behind the target card is revealed to be 10. Similar to the \textit{Position} condition, the \textbf{close distance} character's card was close in physical distance, but far in numerical distance; while the \textbf{close number} character's card was close in numerical distance, but far in physical distance. Participants then attributed emotions to the two characters, and made a forced-choice judgment about who felt worse. There were similar ``Close Distance vs. Far Both" and ``Close Number vs. Far Both" trials.
